...
void draw() {
    ...
    for(int i = 0; i < clockX.length; i++){
      ...
      if(isHit(soldierX[i],  soldierY[i], SOIL_SIZE, SOIL_SIZE, playerX, playerY , SOIL_SIZE, SOIL_SIZE)){			
		...
		// Requirement #6:
		drawCaution(); //Call drawCaution() to draw caution sign
...
String convertFramesToTimeString(int frames){	// Requirement #4
  return nf(floor(frames/3600),2)+":"+nf(floor(frames%3600/60),1,0);
}

color getTimeTextColor(int frames){	
  color timeTextColor = #ffffff;			// Requirement #5
	if(frames>=7200) timeTextColor = #00ffff;
  else if(frames<7200 && frames>=3600) timeTextColor = #ffffff;
  else if(frames<3600 && frames>=1800) timeTextColor = #ffcc00;
  else if(frames<1800 && frames>=600) timeTextColor = #ff6600;
  else if(frames<600) timeTextColor = #ff0000;
...
void drawCaution(){								// Requirement #6
  ...
  if(getEnemyIndexByRow(playerRow+5)>0){
